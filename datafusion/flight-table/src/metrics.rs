        ...
        Self {
            time_creating_client: StartableTime {
                metrics: MetricBuilder::new(metrics)
                    .subset_time("time_creating_client", partition),
                start: None,
            },
            time_getting_stream: StartableTime {
                metrics: MetricBuilder::new(metrics)
                    .subset_time("time_getting_stream", partition),
                start: None,
            },
        }
    }
    ...
    pub(crate) fn new(metrics: &ExecutionPlanMetricsSet, partition: usize) -> Self {
        Self {
            time_processing: StartableTime {
                metrics: MetricBuilder::new(metrics)
                    .subset_time("time_processing", partition),
                start: None,
            },
            time_reading_total: StartableTime {
                metrics: MetricBuilder::new(metrics)
                    .subset_time("time_reading_total", partition),
                start: None,
            },
