...
use arrow_array::{
    builder::StringDictionaryBuilder, cast::AsArray, types::UInt16Type, Array,
...
fn gc_batch(batch: RecordBatch) -> RecordBatch {
    let new_columns = batch.columns().into_iter().map(|column| {
        if let Some(dict_array) = column.as_dictionary_opt::<UInt16Type>() {
            if let Some(typed_dict_array) = dict_array.downcast_dict::<StringArray>() {
                ...
                if values_len > array_len {
                    let mut gc_array =
                        StringDictionaryBuilder::<UInt16Type>::with_capacity(
