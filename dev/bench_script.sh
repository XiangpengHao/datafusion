...
do
    # Define the command
    command="cargo run --profile release-nonlto --bin dfbench -- clickbench --queries-path benchmarks/queries/clickbench/queries.sql --iterations 4 --path benchmarks/data/hits.parquet --query $i --pushdown-filters --flight-cache http://localhost:50051"
